...
def audio_process_task(self, job_id: str, input_path: str, user_id: str, options: dict):
    ...
    try:
        ...
        if output_bytes:
            ...
            mime_map = {"mp3": "audio/mpeg", "wav": "audio/wav", "aac": "audio/aac", "ogg": "audio/ogg", "flac": "audio/flac"}
            out_mime = mime_map.get(output_ext, "audio/mpeg")
            storage.upload_sync(output_bytes, out_path, out_mime)
            output_file_id = _create_output_file(
                db, user_id, out_filename, out_path, out_mime, len(output_bytes), operation, "audio"
            )
