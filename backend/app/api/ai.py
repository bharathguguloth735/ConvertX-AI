...
async def ask_document(
    ...
    """Ask a question about an indexed document (RAG - SQLite and Postgres compatible)."""
    ...
    import re
    q_words = [w for w in re.findall(r'\w{3,}', data.question.lower()) if w not in {"what", "when", "where", "which", "how", "does", "have", "this", "that"}]
    
    if len(chunks) <= 6:
        top_chunks = list(chunks)
    else:
        scored_chunks = []
        for c in chunks:
        ...
        top_chunks = [c for _, c in scored_chunks[:6]]
        if not top_chunks:
            top_chunks = list(chunks[:6])
